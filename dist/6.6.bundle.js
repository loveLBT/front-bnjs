	...
	var EditPwd = function (_Component) {
		...
		_createClass(EditPwd, [{
			...
			value: function handleTouchEnd() {
				...
				var _state = this.state;
				var oldpwd = _state.oldpwd;
				var newpwd = _state.newpwd;
				var confirmpwd = _state.confirmpwd;
				var actions = this.props.actions;

				var reg = /^\w{6,20}$/;
				if (oldpwd == "") {
					_components.Toast.tip("请输入原密码");
					return false;
				}
				if (newpwd == "") {
					_components.Toast.tip("请输入新密码");
					return false;
				}
				if (!reg.test(newpwd)) {
					_components.Toast.tip("新密码由6-20个字母数字下划线组成");
					return false;
				}
				if (confirmpwd == "") {
					_components.Toast.tip("请再次输入新密码");
					return false;
				}
				...
				this.timer = setTimeout(function () {
					actions.fetchPosts("editpwd", editpwdUrl).then(function (data) {
						...
						if (data.posts.status === "success") {
							...
							_components.Toast.tip("修改失败：" + data.posts.result.message);
						}
					});
				}, 2000);
			}
