...
/***/ function(module, exports, __webpack_require__) {
	...
	var Register = function (_Component) {
		...
		function Register(props) {
			...
			_this.state = {
				loading: false,
				formArry: [{ key: "UserName", value: "", type: "tel", tip: "手机号码格式不正确", reg: /^1[34578]\d{9}$/, placeholder: "手机号", hasChildBorder: true, iconName: "icon_phone", maxlength: "11" }, { key: "TrueName", value: "", type: "text", tip: "名字由2-5个汉字组成", reg: /^[\u4E00-\u9FA5]{2,5}$/, placeholder: "真实姓名", hasChildBorder: true, iconName: "icon_user", maxlength: "20" }, { key: "Password", value: "", type: "password", tip: "密码由6-20个字母数字下划线组成", reg: /^\w{6,20}$/, placeholder: "密码", hasChildBorder: true, iconName: "icon_pwd", maxlength: "20" }, { key: "WeiXingH", value: "", type: "text", tip: "微信号由英文数字组成", reg: /^[A-Za-z0-9]+$/, placeholder: "微信号", hasChildBorder: true, iconName: "icon_weixin", maxlength: "20" }, { key: "UpPhone", value: "", type: "tel", tip: "上级手机号码格式不正确", reg: /^1[34578]\d{9}$/, placeholder: "推荐人手机号", hasInputBorder: true, iconName: "icon_person", maxlength: "11" }],
				AgentID: "2"
			};
			return _this;
		}
		...
		_createClass(Register, [{
			...
			value: function handleChange(name, val) {
				var formArry = this.state.formArry;

				var arry = formArry;
				arry.map(function (item, i) {
					if (item.key == name) {
						item.value = val;
					}
				});
			...
			key: 'handleChangeRadioState',
			value: function handleChangeRadioState(name, val) {
				var newState = {};
				newState[name] = val;
			...
			key: 'getRegisterUrl',
			value: function getRegisterUrl(cb) {
				var _state = this.state;
				var formArry = _state.formArry;
				var AgentID = _state.AgentID;

				var registerUrl = apiUrl + "/WSRegisterUser?";
				for (var i = 0; i < formArry.length; i++) {
					if (formArry[i].value == "") {
						_components.Toast.tip(formArry[i].placeholder + "不能为空");
						cb(false);
						return false;
					} else if (!formArry[i].reg.test(formArry[i].value)) {
						_components.Toast.tip(formArry[i].tip);
						cb(false);
						return false;
					}
				...
				if (!!register) {
					return register.result.map(function (item, i) {
						return _react2.default.createElement(_components.Radio, {
							...
							txt: item.name,
							value: item.agentID,
							handleChange: _this2.handleChangeRadioState.bind(_this2, "AgentID"),
			...
			value: function handleSubmit() {
				...
				var _state2 = this.state;
				var formArry = _state2.formArry;
				var AgentID = _state2.AgentID;

				this.getRegisterUrl(function (url) {
					if (url) {
						_this3.setState({ loading: true });
						_Auth2.default.login(url, function (res) {
							_this3.setState({ loading: false });
			...
			value: function render() {
				...
				return _react2.default.createElement(
					...
					register && _react2.default.createElement(
						'form',
						{ action: '' },
						formArry.map(function (item, i) {
							return _react2.default.createElement(_components.Input, {
								key: i,
								handleChange: _this4.handleChange.bind(_this4, item.key),
