	...
	var BuyProduct = function (_Component) {
		...
		_createClass(BuyProduct, [{
			...
			value: function handleTouchEnd() {
				...
				var submitRetailProductUrl = apiUrl + "/WSRetailSellOrder?productId=" + ProductId + "&count=" + count;
				this.setState({
					loading: true
				});
				this.timer = setTimeout(function () {
					actions.fetchPosts("submitRetailProduct", submitRetailProductUrl).then(function (data) {
						_this2.setState({ loading: false });
						if (data.posts.status === "success") {
							_components.Toast.tip("零售成功");
							_this2.props.router.push('/myretail');
						} else {
							_components.Toast.tip("零售失败");
						}
					});
				}, 2000);
			}
			...
			value: function render() {
				...
				return _react2.default.createElement(
					...
					_react2.default.createElement(
						...
						items[index] && _react2.default.createElement(
							...
							items.map(function (item, i) {
								return _react2.default.createElement(_reactRouter.Link, { key: i, className: 'slider-item', style: { backgroundImage: 'url(' + hostUrl + item.pic + ')' } });
							})
						),
						_react2.default.createElement(
							'div',
					...
					this.state.loading && _react2.default.createElement(_components.Loading, { text: '\u6B63\u5728\u63D0\u4EA4\uFF0C\u8BF7\u7B49\u5F85' })
				);
			}
		}]);
